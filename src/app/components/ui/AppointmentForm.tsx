...
export const AppointmentForm: React.FC<AppointmentFormProps> = ({ className = '', id }) => {
  ...
  const fieldClass = 'w-full h-11 min-w-0 px-3 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B0008] focus:border-transparent bg-white';
  const iconFieldClass = 'w-full h-11 min-w-0 pl-9 pr-3 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B0008] focus:border-transparent bg-white';

  return (
    <form
      id={id}
      onSubmit={handleSubmit}
      className={`bg-white rounded-2xl shadow-xl p-5 sm:p-6 md:p-7 w-full max-w-[430px] mx-auto lg:mx-0 ${className}`}
    ...
    >
      ...
      <div className="space-y-3.5">
        <div>
          <label className="block text-xs font-medium text-[#6B5F5F] mb-1.5">
            Serviço
          </label>
          <select
        ...
        <button
          type="submit"
          className="w-full px-5 py-3 bg-[#8B0008] text-white rounded-lg text-sm sm:text-base font-semibold hover:bg-[#6B0006] transition-all mt-2"
        >
